...
/**
 * Observational mutation-testing gate (Bob-style变异测试).
 * Non-blocking on purpose: the per-file 100% coverage gate already forces a
 * test for every branch, so mutation testing is the complementary "do the tests
 * catch injected bugs?" signal. It stays `allowFailure` until the existing
 * source clears a chosen mutation-score band, then the band can be enforced.
 */
function ciMutationGates(): Gate[] {
  return [
    pnpmScript('mutation', 'test:mutation', {
      label: 'mutation testing (observational)',
      allowFailure: true,
      streamOutput: true,
    }),
  ]
