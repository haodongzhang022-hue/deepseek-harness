...
export const ULID = z.string().ulid() as z.ZodType<ULID>
...
// ──────────────────────────────────────────────
// Zod Schemas (用于运行时验证与序列化)
// ──────────────────────────────────────────────

export const MindMapNodeSchema = z.object({
  id: ULID,
  kind: z.enum(['issue', 'pr', 'decision', 'research', 'review', 'test', 'deploy', 'incident', 'meta']),
  title: z.string().min(1).max(200),
  body: z.string().max(50000),
  status: z.enum(['open', 'in_progress', 'review', 'merged', 'closed', 'blocked', 'cancelled']),
  assignee: z.object({
    type: z.enum(['human', 'subagent', 'skill', 'system']),
    id: z.string(),
  ...
  approvals: z.array(z.object({
    id: z.string(),
    type: z.enum(['tool', 'permission', 'plan', 'custom']),
  ...
  edgeTypes: z.array(z.enum(['derives_from', 'blocks', 'duplicates', 'relates_to', 'fixes', 'tests', 'depends_on'])).default([]),
...
export const MindMapEdgeSchema = z.object({
  ...
  sourceId: ULID,
  targetId: ULID,
  type: z.enum(['derives_from', 'blocks', 'duplicates', 'relates_to', 'fixes', 'tests', 'depends_on']),
...
export type MindMapNodeInput = z.input<typeof MindMapNodeSchema>
export type MindMapEdgeInput = z.input<typeof MindMapEdgeSchema>