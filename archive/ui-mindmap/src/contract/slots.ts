...
import { Slot } from '@deepseek-ai/dsh-client-ui-slots'
import type {
  MindMapNode,
  MindMapEdge,
  NodeKind,
 ...
 */
export interface MindMapViewSlot extends Slot {
  id: 'mindmap'
 ...
 * 大纲面板 - 树形结构视图
 */
export interface MindMapOutlineSlot extends Slot {
  id: 'mindmap-outline'
 ...
 * 搜索面板
 */
export interface MindMapSearchSlot extends Slot {
  id: 'mindmap-search'
 ...
 * 历史/时间旅行面板
 */
export interface MindMapHistorySlot extends Slot {
  id: 'mindmap-history'
 ...
 */
export interface MindMapNodeRendererSlot extends Slot {
 ...
 * 节点右键菜单扩展
 */
export interface MindMapNodeContextMenuSlot extends Slot {
  id: string
  label: string
  icon?: string
  // 可选：仅对特定 kind 生效
 ...
 * 节点悬浮详情扩展 (hover card)
 */
export interface MindMapNodeHoverSlot extends Slot {
  id: string
  kinds?: NodeKind[]
 ...
 * 边渲染器扩展
 */
export interface MindMapEdgeRendererSlot extends Slot {
 ...
 * 顶部工具栏动作
 */
export interface MindMapToolbarActionSlot extends Slot {
  id: string
  label: string
  icon: string
  tooltip?: string
...
// 类型守卫
export function isMindMapViewSlot(slot: Slot): slot is MindMapViewSlot {
  return slot.id === 'mindmap'
}

export function isMindMapNodeRendererSlot(slot: Slot): slot is MindMapNodeRendererSlot {
  return slot.id.startsWith('mindmap.node.')
}

export function isMindMapEdgeRendererSlot(slot: Slot): slot is MindMapEdgeRendererSlot {
