...
**Mind-Map Conversation Canvas** — a spatial, node-graph replacement for the linear chat transcript. Every node is an *Issue* or *PR* with full DSH capability attachment (tools, subagents, goals, approvals, files, snapshots). The canvas is the primary conversation surface; the legacy chat view becomes a *lens* onto the same durable session log.

---

## Core Concept

| Legacy Chat | Mind-Map Canvas |
|-------------|-----------------|
| Linear, time-ordered bubbles | Spatial, semantic graph (Issue→PR→Issue…) |
| One active turn at a time | Multiple concurrent *workstreams* (branches) |
| Scroll to find context | Zoom/pan + semantic search + mini-map |
| Implicit parent/child via fork | Explicit edges with typed relations |
| Session = one transcript | Session = one **canvas** (persisted, shareable) |
...
## Persistence Schema (session-persistence 兼容)
...
## Migration Path (从 Chat 视图)

1. **Read-only Mirror** — 现有 session 自动投影为 mindmap (heuristic: turn → issue, tool-call → pr)
2. **Dual View** — Chat + MindMap 标签页共存，双向同步选中
3. **Default Flip** — 新会话默认打开 MindMap，Chat 降级为 Lens
4. **Legacy Import** — 旧 session 一键转换，保留完整历史
